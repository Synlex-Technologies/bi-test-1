  ...
  const processLedgerData = (jsonData) => {
    ...
    const cleanData = jsonData.map(row => {
      return {
        ...
        ActualQty: row['Actual Qty'] ? parseFloat(row['Actual Qty'].split(' ')[0]) : 0,
    ...
    const vendorVolumeAnalysis = _.chain(cleanData)
      ...
      .map((records, vendorName) => {
        const totalAmount = records.reduce((sum, record) => sum + Math.abs(parseAmount(record.Amount)), 0);
        const totalItems = records.reduce((sum, record) => sum + record.ActualQty, 0);
    ...
    const purchaseTrendAnalysis = _.chain(cleanData)
      .filter(record => record['Voucher Number'] && record['Voucher Number'].toString().includes('/'))
      .map(record => {
        ...
        <div className="bg-white p-4 border border-gray-200 shadow-md rounded">
          ...
          {payload.map((entry, index) => (
            <p key={index} style={{ color: entry.color }}>
              {entry.name}: {entry.name.includes('amount') || entry.name === 'totalAmount' 
    ...
    <div className="p-4 bg-gray-50 min-h-screen">
      ...
      {dashboardData ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white p-4 rounded shadow">
              ...
              <p className="text-3xl font-bold text-blue-600">{dashboardData.topVendors.length}</p>
            </div>
            <div className="bg-white p-4 rounded shadow">
              <h2 className="text-lg font-semibold mb-2">Total Products</h2>
              <p className="text-3xl font-bold text-green-600">{dashboardData.productCategories.length}</p>
            </div>
            <div className="bg-white p-4 rounded shadow">
              <h2 className="text-lg font-semibold mb-2">Total Purchase Value</h2>
              <p className="text-3xl font-bold text-amber-600">
                {formatCurrency(_.sumBy(dashboardData.topVendors, 'totalAmount'))}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded shadow mb-6">
            <h2 className="text-lg font-semibold mb-4">Top 10 Vendors by Purchase Value</h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Legend />
                  <Bar dataKey="totalAmount" name="Purchase Amount" fill="#0088FE" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white p-4 rounded shadow">
              <h2 className="text-lg font-semibold mb-4">GST Registration Types</h2>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}
                    >
                      {dashboardData.gstRegistrationTypes.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip content={<CustomTooltip />} />
                    <Legend />
                    <Area type="monotone" dataKey="amount" name="Purchase Amount" stroke="#8884d8" fill="#8884d8" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded shadow">
              <h2 className="text-lg font-semibold mb-4">Purchase Quantity Distribution</h2>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                  ...
                  >
                    ...
                    <Tooltip content={<CustomTooltip />} />
                    <Legend />
                    <Bar dataKey="value" name="Number of Transactions" fill="#82ca9d" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {/* Purchase Value Distribution */}
          <div className="bg-white p-4 rounded shadow mb-6">
            <h2 className="text-lg font-semibold mb-4">Purchase Value Distribution</h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                ...
                >
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Bar yAxisId="left" dataKey="value" name="Number of Transactions" fill="#FF8042" />
                  <Bar yAxisId="right" dataKey="amount" name="Purchase Amount" fill="#0088FE" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded shadow">
            ...
            <ul className="list-disc pl-5 space-y-2">
              <li><strong>Top Vendor:</strong> {dashboardData.topVendors[0]?.vendorName || 'N/A'} - {formatCurrency(dashboardData.topVendors[0]?.totalAmount || 0)}</li>
              <li><strong>Main Product Category:</strong> {dashboardData.productCategories[0]?.name || 'N/A'} - {formatCurrency(dashboardData.productCategories[0]?.value || 0)}</li>
              <li><strong>Average Transaction Value:</strong> {formatCurrency(_.sumBy(dashboardData.topVendors, 'totalAmount') / dashboardData.purchaseValueDistribution.reduce((sum, item) => sum + item.value, 0))}</li>
              <li><strong>Most Common Purchase Quantity:</strong> {_.maxBy(dashboardData.purchaseQtyDistribution, 'value')?.name || 'N/A'}</li>
              <li><strong>Most Common Purchase Value Range:</strong> {_.maxBy(dashboardData.purchaseValueDistribution, 'value')?.name || 'N/A'}</li>
            </ul>
          </div>
        </>
      ) : (
        <div className="bg-white p-8 rounded shadow text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold text-gray-700">No Dashboard Data Yet</h2>
          <p className="mt-2 text-gray-600">Upload an Excel file to generate the dashboard visualizations.</p>
        </div>
      )}
    </div>
  );
